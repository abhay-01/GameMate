...
const AllGames = () => {
  ...
  // Function to handle accepting the invite and creating the match
  const handleAcceptInvite = async (inviteType, inviteUrl) => {
    try {
      const response = await fetch(
        "https://gamemateserver3-eresf4e6c0drdnaf.southindia-01.azurewebsites.net/accept-matchmaking",
        {
          ...
          body: JSON.stringify({
            sender: userEmail, // or the correct value
            target: friendName, // or the correct value
            url: inviteUrl,
            type: inviteType,
          }),
        }
      ...
      if (response.ok) {
        ...
        // Emit Socket event for matchmaking
        ...
        if (createMatch.ok) {
          ...
          const url = inviteUrl + `?email=${userEmail}`;
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 py-4 gap-x-2 gap-y-8 lg:ml-[10vh]">
          {availableGames.map((game) => (
            <div
              ...
              onClick={() => handleAcceptInvite(game.title, game.url)} // Accept invite on click
            >
              <div className="p-2">
                <img
                  src={game.image}
